...
1. In App 1, call [Create Credential Code](/api-reference/auth/create-credential) to get a one-time code. The code will only be valid for 1 minute. This endpoint requires a [User Action Flow](/api-reference/auth/signing-flows), so it involves signing first **with an existing valid credential** to get an user action token.
