...
<Note>
Check [request-headers](request-headers "mention") for more details about which headers are required
</Note>
...
* Key Credentials -> "manually" generate keypairs yourself, and store them however you see fit (see [How to generate a keypair](credentials/generate-a-key-pair)). You can use Key Credential if you need a Service Account sitting in your server to also be the signer for example.
