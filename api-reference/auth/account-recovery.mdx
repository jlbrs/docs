...
Both `Employee` and `Enduser` user types can recover their Dfns accounts by creating recovery credentials.  `Employees` connected with your organization are provided a recovery kit when they register on the Dfns dashboard.  They should store this recovery kit securely offline.  If they need to recover their account credentials, they can do so at [app.dfns.io/recover](app.dfns.io/recover) by providing the information from the kit and a verification code proving they still have access to their email.  Alternatively, an existing organization user with administrative permissions can deactivate and re-create their account using a different email address.

It is possible to create as many recovery credentials as you want. Any recovery credential can be used to recover a User.

Once a recovery credential is used, all the user credentials (recovery and regular) are invalidated for security reasons. The user needs to create at least one new recovery credential, this can be done during the recovery process (see [Recover User](user-recovery/createuserrecovery)).
...
1. Construct the `newCredential` object, using the challenge obtained from either the [Create Recovery Challenge](createuserrecoverychallenge) or [Create Delegated Recovery Challenge](delegatedrecovery) API.
...
* Call the [delegated recovery](user-recovery/delegatedrecovery) endpoint from a service account (`DfnsApiClient.auth.createDelegatedUserRecovery` in the SDK).
...
* The user signs the new credential(s) with their existing recovery credential, and sends the new credential(s) + signature to Dfns to [Recover User](user-recovery/createuserrecovery) endpoint (`DfnsDelegatedApiClient.auth.createUserRecovery` in the SDK)
...
<Frame>![](https://3953120539-files.gitbook.io/~/files/v0/b/gitbook-x-prod.appspot.com/o/spaces%2FtnSPOZGQ2hBmgoVWX5H6%2Fuploads%2FnaEhMjAQ7eJ3WXQoMvHo%2FRecover%20User%20-%20Diagram.jpg?alt=media&#x26;token=9431d5c5-6c28-4625-b953-c376a87b993e)</Frame>
