...
1. Construct the `newCredential` object, using the challenge obtained from either the [Create Recovery Challenge](/api-reference/auth/create-recovery-challenge) endpoint.
...
4. Recover User: use `POST /auth/recover/user` to recover a user, using a recovery credential. The `recovery.credentialAssertion.clientData` field's challenge must be the _base64url-encoded_ representation of the `newCredential` object.
...
* Call the [Create Delegated Recovery Challenge](/api-reference/auth/create-delegated-recovery-challenge) endpoint from a service account (`DfnsApiClient.auth.createDelegatedUserRecovery` in the SDK).
...
* The user signs the new credential(s) with their existing recovery credential, and sends the new credential(s) + signature to Dfns to [Recover](/api-reference/auth/recover-user) endpoint (`DfnsDelegatedApiClient.auth.createUserRecovery` in the SDK)
...
<Frame>![](/images/spaces_2FtnSPOZGQ2hBmgoVWX5H6_2Fuploads_2FnaEhMjAQ7eJ3WXQoMvHo_2FRecover_20User_20-_20Diagram.jpg)</Frame>
