...
* The user signs the new credential(s) with their existing recovery credential, and sends the new credential(s) + signature to Dfns to [Recover](/api-reference/auth/recover) endpoint (`DfnsDelegatedApiClient.auth.createUserRecovery` in the SDK)
