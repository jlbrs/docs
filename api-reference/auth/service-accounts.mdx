...
As any user, they must sign their API requests (cf [User Action Signing](/api-reference/auth/signing-flows)). So before creating a Service Account, an asymmetric keypair must be created. It will be used as the Service Account [Credential](/api-reference/auth/credentials). The public part of this key is passed during the Service Account creation.
