...
There are many ways to authenticate to the Dfns API, you can find all the details on [authentication](/api-reference/auth). In this example we will simply follow the normal [login](/api-reference/auth/login-flows#regular-user-login-flow) flow.
...
```
    As explained in [Create Credential With Code Flow](/api-reference/auth/credentials#create-credential-with-code-flow), in order to register the credentials we need to
...
```
  ...
  <Step title="Login using your 🔑 Passkey">
    In a very similar way, as explained in [login](/api-reference/auth), you need to request a Bearer token for authenticating all your API requests.
...
```
  ...
  <Step title="User Actions: signing requests">
    ...
    For any action that's modifying things in your account (e.g.: creating a wallet), Dfns asks for a signature, which is done by using your 🔑 passkey as earlier with the login. As before this process requires multiple steps:
   
    1. Get a "User Action Signature Challenge" from `POST /auth/action/init`. See details on [inituseractionsigning](/api-reference/auth/create-user-action-challenge).
