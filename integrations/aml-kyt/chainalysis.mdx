...
Once that is done, the integration is done through the [Policy Engine](/core-concepts/policies).
...
* [activity kind `Wallets:Sign`](/core-concepts/policies#wallets%3Asign-activity)
* [rule kind `ChainalysisTransactionPrescreening`](/core-concepts/policies#chainalysistransactionprescreening-policy-rule)
* action kind [`Block`](/core-concepts/policies#block-policy-action) or [`RequestApproval`](/core-concepts/policies#requestapproval-policy-action) or [`NoAction`](/core-concepts/policies#noaction-policy-action)

When using policy rule `ChainalysisTransactionPrescreening`, the transfer attempt will be sent to Chainalysis for a KYT screening. If the screening result triggers the rule (based on the criterias you defined in the rule), then the details of the Chainalysis pre-screening results will be published in the   [Webhook Event](/developers/webhooks#webhook-events) `policy.triggered`, which is published for any policy which triggers.
...
* [activity kind `Wallets:IncomingTransaction`](/core-concepts/policies#wallets-incomingtransaction-activity)
* [rule kind `ChainalysisTransactionScreening`](/core-concepts/policies#chainalysistransactionscreening-policy-rule)
* action kind [`NoAction`](/core-concepts/policies#noaction-policy-action)
