...
The Fee Sponsor enables you to sponsor gasless transactions for your custodial wallets via the [Transfers API](wallets/transfer-asset). This allows you to manage a small set of fee sponsor wallets with native tokens and use them to pay gas fees for other wallets.

To sponsor a transfer,

* [Designate](fee-sponsors/create-fee-sponsor) a Dfns wallet as the fee sponsor. See below for supported networks.
* Fund the fee sponsor wallet with some native tokens.
* Specify the `feeSponsorId` when [creating a transfer request](wallets/transfer-asset). Note that the sponsor and sponsoree wallets must be for the same network.
