...
There are many ways to authenticate to the Dfns API, you can find all the details on [authentication](../../advanced-topics/authentication "mention"). In this example we will simply follow the normal [login](../../api-docs/authentication/login) flow.&#x20;
...
As explained in [#create-credential-with-code-flow](../../../advanced-topics/authentication/credentials#create-credential-with-code-flow "mention"), in order to register the credentials we need to&#x20;
...
In a very similar way, as explained in [login](../../api-docs/authentication/login "mention"), you need to request a Bearer token for authenticating all your API requests.&#x20;
...
1. Get a "User Action Signature Challenge" from `POST /auth/action/init`. See details on [inituseractionsigning](../../api-docs/authentication/user-action-signing/inituseractionsigning "mention").
